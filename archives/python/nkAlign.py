...
def getCollides(point, bb):
    '''return whether a point is within a node's screen bounding box. NOT YET IMPLEMENTED'''
    print("this does nothing")
...
def alignChain():
    '''aligns the B-pipe (0th input) chain. NOT IMPLEMENTED'''
...
def nudgeWithBuffer(direction, nodes=None):
    '''nudge the nodes with an awareness of other nodes in the hierarchy. NOT YET IMPLEMENTED'''
    nodes = nodes if nodes else nuke.selectedNodes()
